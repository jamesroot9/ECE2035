...
TEST(AccessTest, AddDuplicate) {
	...
	void* result = insertItem(ht, 0, m[1]);
	// Test if the hash table returns NULL when the key is not found.
	EXPECT_EQ(m[0], result);
	free(result);
...
TEST(RemoveTest, MiddleItemRemove) {
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	// remove middle item from hash table.
	...
	// Check other values still in ht.
	EXPECT_EQ(m[0], getItem(ht, 0));
...
TEST(DeleteTest, SingleDeleteTest) {
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
